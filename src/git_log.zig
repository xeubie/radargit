...
pub fn GitCommitList(comptime Widget: type) type {
    return struct {
        ...
        pub fn input(self: *GitCommitList(Widget), allocator: std.mem.Allocator, key: Key, root_focus: *Focus) !void {
            _ = allocator;
...
pub fn GitLog(comptime Widget: type) type {
    return struct {
        ...
        pub fn input(self: *GitLog(Widget), allocator: std.mem.Allocator, key: Key, root_focus: *Focus) !void {
            ...
            if (self.getFocus().child_id) |child_id| {
                if (self.box.children.getIndex(child_id)) |current_index| {
                    ...
                    const index = inp.horizIndex(key, child.* == .git_commit_list, diff_scroll_x) orelse blk: {
